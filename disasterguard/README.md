...
```
• Built DisasterGuard, a real-time AI-powered disaster alert system
  monitoring 10 Indian cities, processing 120 weather events/hour via
  OpenWeatherMap API with automated risk classification and sub-500ms
  alert generation latency

• Designed event-driven architecture: weather poller → HuggingFace AI
  classifier (zero-shot, bart-large-mnli) with rule-based fallback →
  Supabase PostgreSQL → Server-Sent Events → React Leaflet map, with
  full end-to-end latency under 2 seconds

• Implemented Haversine geodesic distance algorithm for nearest-shelter
  matching across 10 Indian states, handling full-capacity fallback and
  cross-region search with automated Twilio SMS and Gmail SMTP notifications

• Deployed full-stack on free-tier infrastructure (Supabase + Vercel +
  Render) with Supabase Auth, exponential backoff SSE reconnection, and
  per-city fault isolation — system continues operating if any single
  city's API call fails
```
